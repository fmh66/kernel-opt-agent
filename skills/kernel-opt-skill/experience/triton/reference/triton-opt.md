...
**The `order` parameter in `tl.make_block_ptr`** determines traversal direction:
- `order=(1, 0)` → row-major traversal (contiguous along the last dimension) — matches PyTorch default layout
- `order=(0, 1)` → column-major traversal — only use when the tensor is already column-major in memory
- **Rule**: always match `order` to the tensor's actual memory layout. Mismatch causes strided access and bandwidth collapse.

**Triton compiler auto-coalescing**: the compiler automatically identifies contiguity and generates wide vectorized instructions:

| Element Size | Min Contiguity | Generated PTX |
|---|---|---|
| 32-bit (f32) | 16 bytes (4 elements) | `ld.global.v4.b32` |
| 16-bit (f16) | 8 bytes (4 elements) | `ld.global.v2.b32` |
| 8-bit (fp8) | 16 bytes (16 elements) | 4× `ld.global.v4.b32` |

To help the compiler, use `tt.divisibility = 16` on pointer arguments to signal alignment.

### ⚠️ Pitfall: Coalescing vs Bank Conflict Tradeoff

A classic lesson from the Triton matmul tutorial: transposing Y to `.T.contiguous()` makes column loads coalesced, but the subsequent `tl.dot(x, y.T)` transpose in shared memory introduces bank conflicts that can **reduce performance by 50%**. The coalescing gain is offset by the bank conflict loss. **Must verify with NCU** — do not blindly optimize for coalescing alone.
...
**Community-measured gains (Hoque et al., 2024):**
- A100: average **65% speedup** across skinny GEMM shapes
- H100: average **124% speedup**, up to **295% at Split-K=8**
- Achieved Occupancy: 7.6% → 27.8%; SM Utilization: 20.8% → 43.1%
- The key metric Split-K improves is SM utilization — without it, most SMs are idle on skinny shapes

### ML-Triton Multi-Level Tiling (2025)

Latest research (Wang et al., Mar 2025): ML-Triton introduces a multi-level compilation pipeline:
- **Workgroup → Warp → Intrinsic** three-stage progressive lowering
- Developer can specify tiling strategy via compiler hints: horizontal / vertical / square
- Exposes warp-level primitives: `tl.warp_id()`, `tl.alloc`, warp-scoped `tl.reduce`
- On Intel GPU: achieves **95-96%** of expert hand-written kernel performance

This represents the direction Triton is heading — more explicit control over the lowering pipeline for expert users while maintaining the high-level programming model.

> Source: [arXiv:2503.14985 - ML-Triton](https://arxiv.org/abs/2503.14985)
...
| Architecture | Key Features | Tuning Notes |
|---|---|---|
| **A100** (Ampere) | 192 KB SRAM/SM, 40 MB L2 | Split-K effective; TMA not available; `cp.async` for async loads |
| **H100/H800** (Hopper) | TMA, WGMMA, 228 KB SRAM/SM | Use `cp.async.bulk` for async copy; watch swizzle/subview compatibility |
| **H800** (bandwidth-limited H100) | NVLink 200 GB/s (vs H100 450 GB/s) | Must do compute-communication overlap for multi-GPU |
| **Consumer GPUs** (RTX 4090) | Smaller L2, fewer SMs | Tile sizes and `num_stages` differ significantly from datacenter GPUs |

### FlexAttention Layout Fix (production case study)

PyTorch #153357 (2025): FlexAttention FP8 kernels suffered performance degradation when HBM memory layout prevented shared memory loads from being pipelined. Root cause: row-major to column-major transpose in storage created bank conflicts that broke the compiler's pipelining pass. **Fix**: enforce correct memory layout (column-major for V) outside the kernel, so the compiler can generate pipelined async loads. Lesson: memory layout affects not just coalescing but also the compiler's ability to pipeline — always verify with PTX inspection.

> Source: [PyTorch #153357 - FlexAttention Layout Fix](https://github.com/pytorch/pytorch/commit/881a598a1e38ef06d4f51d1e3fd8e359fed0c3a0) · [FlashAttention Triton Memory Access Analysis](https://blog.hotdry.top/posts/2025/12/24/flashattention-triton-memory-access-register-allocation-optimization/)
