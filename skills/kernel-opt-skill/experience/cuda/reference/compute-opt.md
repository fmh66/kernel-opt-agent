...
### WMMA vs MMA PTX — Choosing the Right Abstraction

| Aspect | WMMA (C++ API) | MMA PTX (Inline Assembly) |
|---|---|---|
| Abstraction | High-level, type-safe `wmma::fragment` | Low-level `mma.sync.aligned` instructions |
| Portability | Portable across architectures | Architecture-specific; fragile across toolchains |
| Performance ceiling | ~95-98% of cuBLAS | Up to ~100% of cuBLAS |
| Control | Compiler manages register allocation | Full control over register mapping and scheduling |
| Development cost | Lower; easier to write and maintain | High; requires deep hardware knowledge |
| Use case | Most production kernels | Peak-performance, competition-grade kernels |

Key insight: on modern GPUs (Ada Lovelace, RTX 4090), well-tuned WMMA actually **outperforms** hand-written PTX on large matrices (MNK ≥ 8192) because the compiler optimizes register allocation and scheduling better than manual approaches. Start with WMMA; graduate to PTX only if NCU shows WMMA overhead is the bottleneck.

### Register Pressure Management for Tensor Core Kernels

Tensor Core fragments consume significant registers. For FP16 WMMA with `m16n16k16` tiles: A fragment = 8 halfs/thread, B fragment = 8 halfs/thread, C fragment = 4 floats (accumulator) per thread. Register pressure grows rapidly with multi-stage pipelines.

**Mitigation strategies (in priority order):**

1. **Dynamic Shared Memory (DSMEM)**: `extern __shared__ half smem[]` — offloads data from registers to SMEM. DSMEM variants typically achieve better occupancy than static SMEM versions, at a slight SMEM cost.
2. **Fragment reuse patterns ("Right → Left → Right → Left")**: alternate access direction across inner K tiles, reducing the number of live register fragments at any point. Cuts peak register pressure **20-30%** without spilling.
3. **`__launch_bounds__(maxThreads, minBlocks)`**: guides compiler to spill strategically rather than blindly overallocating registers.
4. **`-maxrregcount=N` compiler flag**: last resort — algorithmic register reduction is always preferable to forced spilling.
5. **Adapt MMA tile config to problem size**: small matrices (MNK < 8192) → use `mma4x2_warp2x4` (8 warps, lower regs); large matrices (MNK ≥ 8192) → scale up to `mma4x4_warp4x4` (16 warps, higher regs).

### Multi-Stage Software Pipelining

The dominant ILP technique for Tensor Core kernels — overlaps global→shared memory transfers with computation:

| Stage Count | Buffering | Use Case |
|---|---|---|
| 2 (double) | 2 SMEM buffers, baseline | Memory-limited scenarios (large tiles) |
| 3 (triple) | 3 SMEM buffers, balanced | Optimal for most workloads |
| 4 (quad) | 4 SMEM buffers, maximum overlap | Compute-bound small tiles |

Conceptual 3-stage pipeline:
```
Stage 1: cp.async(smem_stage[0], gmem_tile[0]);  // load tile 0
Stage 2: cp.async(smem_stage[1], gmem_tile[1]); mma_sync(frag, smem_stage[0]); // load tile 1, compute tile 0
Stage 3: cp.async(smem_stage[2], gmem_tile[2]); mma_sync(frag, smem_stage[1]); // load tile 2, compute tile 1
```

Stage selection depends on SMEM budget: compute `tile_size * element_bytes * 2 (A+B) * num_stages` and ensure it fits in available SMEM.

### Block Swizzle Stride Heuristic

Block swizzling reorders thread block execution to improve L2 cache hit rate:
```python
if N <= 4096:      stride = N / 2
elif N < 14848:    stride = N / 4
else:              stride = N / 8   # large matrices
```

> Sources: [Bruce-Lee-LY/cuda_hgemm](https://github.com/Bruce-Lee-LY/cuda_hgemm) (production-quality WMMA/MMA implementations achieving ≥95% cuBLAS) · [NVIDIA Tensor Core Programmability](https://www.emergentmind.com/topics/nvidia-tensor-core-programmability) · [Advanced GPU Optimization: Tensor Core Programming](https://dev.to/javadinteger/advanced-gpu-optimization-tensor-core-programming-nvidia-3708)
...
### Ada Lovelace GEMM Performance (community-validated)

From an ML-based study on RTX 4070 analyzing 16,128 CUTLASS GEMM configurations:
- Optimal tile size selection (16×16) yields up to **3.2× performance improvement** and **22% power reduction** vs suboptimal tile sizes
- A Random Forest model achieved R²=0.98 for runtime prediction (15.57% mean error) — tile size × occupancy interaction is the dominant performance factor
- Key takeaway: tile size tuning is not just about raw throughput — it also significantly affects energy efficiency

> Source: [arXiv:2411.16954 - GEMM Performance on Ada Lovelace](https://arxiv.org/abs/2411.16954)
...
- **Register pressure**: check register usage and local memory spills before/after; verify that occupancy change aligns with the expected direction (compute-bound: lower occupancy is acceptable if IPC improves).
...
- Using MMA PTX when WMMA would suffice — the development cost premium of PTX is only justified when NCU shows WMMA overhead is the top bottleneck.
