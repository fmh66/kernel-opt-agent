...
def find_cutlass_roots() -> list[Path]:
    roots: list[Path] = []
    for env_name in ("CUTLASS_PATH", "CUTLASS_ROOT", "CUTLASS_HOME"):
        value = os.environ.get(env_name)
        if value:
...
def probe_cutlass_headers() -> dict[str, Any]:
    headers = [
    ...
    searched: list[str] = []
    for root in find_cutlass_roots():
        searched.append(str(root))
        if all((root / header).exists() for header in headers):
            return {
                "exists": True,
    ...
    return {
        "exists": False,
        "root": "",
        "detail": "set CUTLASS_PATH/CUTLASS_ROOT/CUTLASS_HOME to a CUTLASS source/install root containing include/cutlass and include/cute",
        "searched": searched,
    }
...
def collect_env_check(gpu_index: int) -> dict[str, Any]:
    ...
    implementation_status: dict[str, dict[str, Any]] = {}
    ...
    def add_base_req(name: str, ok: bool, detail: str) -> None:
        add_req(name, ok, detail, required=True)
        if not ok:
            base_errors.append(f"{name}: {detail}")

    def add_impl_req(name: str, ok: bool, detail: str) -> None:
        add_req(name, ok, detail, required=False)
    ...
    add_base_req("PyTorch import", torch_info["importable"],
                 torch_info["version"] if torch_info["importable"] else (torch_info["error"] or "torch import failed"))
    add_base_req("CUDA runtime", torch_info["cuda_available"],
    ...
    add_base_req(f"GPU index {gpu_index}", selected_gpu_ok,
    ...
    add_impl_req("nvcc executable", nvcc_info["exists"], nvcc_info["resolved"] or "cannot resolve nvcc")
    ...
    result["ncu"] = ncu_info
    add_base_req("ncu executable", ncu_info["exists"], ncu_info["resolved"] or "cannot resolve ncu")
    ...
    add_base_req("nsight-python package", nsight_info["importable"],
    ...
    add_impl_req("triton package", triton_info["importable"],
                 f"triton {triton_info['version']}" if triton_info["importable"] else (triton_info["error"] or "import triton failed"))

    cutlass_python_info = probe_python_module("cutlass")
    result["cutlass_python"] = cutlass_python_info
    add_impl_req("cutlass python package", cutlass_python_info["importable"],
                 f"cutlass {cutlass_python_info['version']} ({cutlass_python_info['path'] or 'path unknown'})"
                 if cutlass_python_info["importable"] else (cutlass_python_info["error"] or "import cutlass failed"))

    cute_dsl_info = probe_python_module("cutlass.cute")
    result["cute_dsl_python"] = cute_dsl_info
    add_impl_req("cute-dsl python package", cute_dsl_info["importable"],
    ...
    cutlass_headers = probe_cutlass_headers()
    result["cutlass_headers"] = cutlass_headers
    add_impl_req("CUTLASS headers", cutlass_headers["exists"],
                 cutlass_headers["root"] if cutlass_headers["exists"] else cutlass_headers["detail"])

    implementation_status.update({
        "cuda-cpp": {
            "ready": bool(nvcc_info["exists"]),
            "requirements": ["nvcc executable"],
            "detail": nvcc_info["resolved"] or "missing nvcc",
        },
        "cutlass": {
            "ready": bool(nvcc_info["exists"] and cutlass_headers["exists"]),
            "requirements": ["nvcc executable", "CUTLASS headers"],
            "detail": cutlass_headers["root"] if cutlass_headers["exists"] else cutlass_headers["detail"],
        },
        "cute-dsl": {
            "ready": bool(cute_dsl_info["importable"]),
            "requirements": ["cutlass.cute python package"],
            "detail": cute_dsl_info["path"] if cute_dsl_info["importable"] else (cute_dsl_info["error"] or "import cutlass.cute failed"),
        },
        "triton": {
            "ready": bool(triton_info["importable"]),
            "requirements": ["triton package"],
            "detail": f"triton {triton_info['version']}" if triton_info["importable"] else (triton_info["error"] or "import triton failed"),
        },
    })
    implementation_ready = any(item.get("ready") for item in implementation_status.values())
    result["implementation_ready"] = implementation_ready
    if not implementation_ready:
        errors.append("implementation backend: at least one of cuda-cpp, cute-dsl, cutlass, or triton must be ready")

    result["ready"] = not base_errors and implementation_ready
    return result
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    for item in result.get("requirements", []):
        ...
        required = "yes" if item.get("required") else "implementation"
        detail = str(item.get("detail", "")).replace("\n", "<br>")
        lines.append(f"| {item.get('name')} ({required}) | {status} | {detail} |")
    ...
    cutlass_headers = result.get("cutlass_headers") or {}
    ...
    for name in ("cuda-cpp", "cute-dsl", "cutlass", "triton"):
        item = implementation_status.get(name, {})
    ...
    lines.extend([
        ...
        f"- cute-dsl python: {cute_dsl_py.get('version') or 'not importable'}",
        f"- CUTLASS headers: {cutlass_headers.get('root') or 'not found'}",
        "",
        ...
        f"- CUTLASS_PATH: {result.get('env_vars', {}).get('CUTLASS_PATH') or '(unset)'}",
        f"- CUTLASS_ROOT: {result.get('env_vars', {}).get('CUTLASS_ROOT') or '(unset)'}",
        f"- CUTLASS_HOME: {result.get('env_vars', {}).get('CUTLASS_HOME') or '(unset)'}",
        "",
        "## Errors",
    ])
