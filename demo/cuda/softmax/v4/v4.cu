...
__global__ void softmax_v4(float* input, float* output, int N, int D) {
    ...
    // Step 2: Compute exp via __expf (fast-math), store to shared, compute sum
    float sum_val = 0.0f;
    for (int i = tid; i < D; i += BLOCK_DIM) {
        float val = __expf(in_row[i] - max_val);
        exp_shared[i] = val;
        sum_val += val;
    }
    __syncthreads();
...
extern "C" void solve(float* input, float* output, int N, int D) {
    ...
    softmax_v4<<<N, BLOCK_DIM, smem_bytes>>>(input, output, N, D);
