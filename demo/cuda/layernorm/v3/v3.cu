    ...
    int N, int D, float eps)
{
    ...
    // --- normalize with __ldg for gamma/beta ---
    for (int i = tid; i < D; i += blockDim.x) {
        float g = __ldg(gamma + i);
        float b = __ldg(beta  + i);
        out_row[i] = (in_row[i] - mean) * inv_std * g + b;
    }
}
    ...
    int N, int D)
{
    ...
    layernorm_v3<<<blocksPerGrid, threadsPerBlock, smem_size>>>(input, gamma, beta, output, N, D, eps);
