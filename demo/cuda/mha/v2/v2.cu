    ...
    int N, int d_model, int h, int d_k)
{
    ...
    // Each thread handles a contiguous chunk of key positions
    int chunk = (N + d_k - 1) / d_k;
    int start_j = tid * chunk;
    int end_j = min(start_j + chunk, N);

    // Compute scores for contiguous chunk — coalesced shared memory writes
