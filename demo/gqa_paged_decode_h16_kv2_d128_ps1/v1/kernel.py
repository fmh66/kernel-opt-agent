...
def _gqa_paged_decode_kernel(
    ...
    if pid_batch >= batch_size or pid_kv >= num_kv_heads:
        return
    ...
    q_offset_base = pid_batch * (num_qo_heads * head_dim) + pid_kv * (GQA_RATIO * head_dim)

    log2e = 1.4426950408889634

    for group_idx in tl.static_range(GQA_RATIO):
        q_head = pid_kv * GQA_RATIO + group_idx
        q_ptrs = q_ptr + q_offset_base + group_idx * head_dim + tl.arange(0, BLOCK_D)
        ...
        for page_idx in range(n_pages):
            ...
            kv_offset = kv_idx * stride_kv_page + pid_kv * head_dim
